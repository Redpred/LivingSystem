...
- **阶段五（毒素/病原体/辐射/魔法）**：毒素/病原体/辐射已完成。
  - 毒素：`ToxinEngine` 药代（吸收→代谢）、负荷汇总、致死阈值；环境 TOXIN 危害经吸入注入剂量，派生恶心症状。示例：凋零毒素 + 凋零玫瑰接触。
  - 病原体：`PathogenEngine` 感染状态机（潜伏→症状→恢复→清除）、载量 vs 免疫对抗、致死载量；开放污染伤口按概率继发伤口感染，环境 PATHOGEN 危害发起感染，派生发热/虚弱。示例：伤口感染。
  - 辐射：`RadiationEngine` 按类型累积剂量并衰减、阈值产生生物效应、致死剂量；环境 RADIATION 危害累积伽马剂量，派生恶心/虚弱。示例：伽马辐射。
  - 剩余：魔法异常（ARCANE，开发文档规格较模糊，待细化）、解毒/免疫增强药物与对应治疗、外部/体内污染细分。
- **阶段六（医疗设备/生产/兼容）**：兼容 API 已完成。
  - 公开兼容 API：`LivingSystemApi` 门面注入 `LivingSystemApiHolder`；其它模组可注册伤害画像/治疗定义/防护画像（并入代码注册区，数据包重载时合并、同 ID 数据包优先），并订阅死亡报告事件（`ApiEventBus`）。
  - 医疗工作站：功能化为储存设施 —— 带方块实体的 9 格医疗物资库存（`ItemStackHandler` 持久化）、容器菜单（Shift 快速转移）、客户端界面、右键打开、破坏掉落库存与自身、合成配方（铁锭+箱子）。
  - 剩余：医疗生产/治疗设备（机器角色/配方/能源接入）、外部科技模组配方接入、完整管理界面、官方兼容数据包。
- **阶段七（平衡/资源/发布）**：未开始。补齐默认平衡数据、贴图/模型/音效/动画资源、配置模板、数据字典、迁移样本、自动化测试、多人压测、兼容矩阵与发布文档（其中美术/音效/动画与多人实测需人工资源，无法由代码生成）。
